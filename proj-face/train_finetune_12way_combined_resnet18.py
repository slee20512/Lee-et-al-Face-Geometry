...
# ImageNet-pretrained fine-tune ablation of train_from_scratch_12way_combined_resnet18.py:
# same *combined* 12-way identity-only dataset -- ConcatDataset of the plain
# vbsle_50k/vbsle_50k_12way set with the vbsle_50k_texture_colorbg/texture_colorbg_12way set
# (same 12 identity classes in both, verified matching folder names -- ImageFolder's
# class_to_idx is computed by sorting folder names, so both instances get identical label
# indices and can be concatenated directly) -- but starting from ImageNet weights (like
# train_finetune_28way_combined_resnet18.py) instead of random init.
...
args.pretrained = True  # start from ImageNet weights, unlike train_from_scratch_12way_combined_resnet18.py
...
args.filename_prefix = f'{args.arch}_finetune_12way_combined_seed{args.seed}_'
