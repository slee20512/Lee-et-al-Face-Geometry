...
# ver2 of train_from_scratch_12way_supervised.py: both the lr=0.1 and lr=0.01
# from-scratch attempts (plain SGD, hard step-decay, no warmup) converged to exactly
# chance-level accuracy/loss over all 100 epochs. This version changes the recipe
# itself rather than just the lr: linear lr warmup (the classic fix for random-init
# training getting stuck early), a cosine schedule instead of hard step-decay
# (matching the SimCLR scripts here), Adam instead of SGD (adaptive per-parameter
# scaling), and much lower weight decay (a strong decay can suppress an already-weak
# gradient signal).
...
args.lr = 1e-3  # Adam lr, not an SGD lr -- much lower than the 0.1/0.01 SGD attempts
args.epochs = 100
args.warmup_epochs = 5

args.filename_prefix = f'{args.arch}_scratch_vbsle_50k_12way_ver2_seed{args.seed}_'
