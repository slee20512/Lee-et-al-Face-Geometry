...
# Curriculum learning, stage 2 (self-supervised branch), temperature ablation: same
# setup as train_curriculum_simclr_7way_EM_dan.py (init from the SimCLR 60-way-IDEM
# backbone, continue NT-Xent training on one identity's 7-way emotion images, lr 3e-5),
# but temperature=0.1 (the SimCLR paper's standard) instead of 0.5. The lr3e-5 and
# lr3e-4 curriculum runs and a from-scratch run all converged to the same ~2.91 loss
# floor regardless of init or lr -- a soft temperature=0.5 gives little gradient signal
# to separate already-similar same-identity negatives, so this checks whether a
# sharper temperature unsticks it.
...
args.filename_prefix = f'{args.arch}_curriculum_simclr_7way_EM_dan_colorbg_from_simclr60way_temp0.1_seed{args.seed}_'
