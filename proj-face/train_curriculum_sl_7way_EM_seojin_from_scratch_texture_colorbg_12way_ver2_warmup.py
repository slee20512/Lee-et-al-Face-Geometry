...
# Recipe ablation of train_curriculum_sl_7way_EM_seojin_from_scratch_texture_colorbg_12way_ver2.py:
# same cross-domain curriculum (textured 12-way ver2 backbone -> geometry-only 7-way
# emotion), but Adam + linear warmup + cosine decay instead of plain SGD +
# MultiStepLR -- the recipe change that broke chance level in the original
# from-scratch saga (train_from_scratch_12way_supervised_ver2.py), being tried here
# since the plain-SGD curriculum version stayed at chance through epoch 1 with an
# lr drop at epoch 10 baked in that would make recovery even less likely.
...
args.filename_prefix = f'{args.arch}_curriculum_sl_7way_EM_seojin_from_scratch_texture_colorbg_12way_ver2_warmup_seed{args.seed}_'
...
if os.path.exists(latest_filename):
    ...
    # saved state_dict, then swap in a fresh 7-way head.
    model = models.resnet50(pretrained=False)
