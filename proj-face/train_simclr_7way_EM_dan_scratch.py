...
# Diagnostic baseline for the dan-curriculum SimCLR runs: random-init SimCLR trained
# directly on one identity's 7-way emotion images, no stage-1 60-way checkpoint at
# all. The curriculum run (train_curriculum_simclr_7way_EM_dan.py) plateaued around
# loss 2.90 from epoch 0; this checks what loss trajectory this narrow, single-identity
# dataset produces on its own -- if it also plateaus near 2.90, that's a real floor
# from harder in-batch negatives (all same identity), not an artifact of continuing
# from an already-converged checkpoint.
...
args.filename_prefix = f'{args.arch}_simclr_7way_EM_dan_colorbg_scratch_seed{args.seed}_'
