...
# Curriculum-training follow-up, item 12, stage 2: reversed-order curriculum
# (emotion -> identity, instead of the original identity -> emotion). Continues the
# stage-1 checkpoint from train_from_scratch_7way_EM_seojin_resnet18.py (resnet18,
# random-init, trained on seojin's 7-way emotion set) onto the 12-way identity task
# (vbsle_50k_12way -- the same target dataset the original-order stage 1 was trained
# on directly). Same lr/epoch schedule as the original order's stage 2
# (train_curriculum_sl_7way_EM_seojin_from_scratch12way_resnet18.py) for direct
# comparability.
...
args.data_root = '/mnt/smb/locker/issa-locker/users/Seojin/data/face_data/vbsle_50k/vbsle_50k_12way/'
args.seed = 777
...
args.filename_prefix = f'{args.arch}_curriculum_sl_12way_from_scratch7way_EM_seojin_seed{args.seed}_'
