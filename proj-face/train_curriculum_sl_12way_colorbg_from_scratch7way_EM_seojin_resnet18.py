...
# Curriculum-training follow-up, item 12, stage 2 (textured-domain variant): reversed-
# order curriculum (emotion -> identity), continuing onto the textured 12-way identity
# set (texture_colorbg_12way) instead of the geometry-only one
# (train_curriculum_sl_12way_from_scratch7way_EM_seojin_resnet18.py). Stage 1
# (train_from_scratch_7way_EM_seojin_resnet18.py) is supervised, not SimCLR, so this
# continuation stays supervised too -- matching the domain-consistent pairing pattern
# already used elsewhere here (e.g.
# train_curriculum_sl_7way_EM_seojin_colorbg_from_scratch_texture_colorbg_12way_ver2.py).
# Same lr/epoch schedule as the geometry-target sibling script for direct comparability.
...
args.stage1_checkpoint = 'resnet18_scratch_7way_EM_seojin_seed777_model_best.pth.tar'
args.stage1_num_classes = 7

args.data_root = '/mnt/smb/locker/issa-locker/users/Seojin/data/face_data/vbsle_50k_texture_colorbg/texture_colorbg_12way/'
args.seed = 777
...
args.filename_prefix = f'{args.arch}_curriculum_sl_12way_colorbg_from_scratch7way_EM_seojin_seed{args.seed}_'
...
assert num_outputs == 12, f"expected 12 identity classes, got {num_outputs}: {train_dataset.classes}"
...
if os.path.exists(latest_filename):
    ...
    # stage-1 checkpoint is a 7-way emotion classifier -- build with that head to match
    # the saved state_dict, then swap in a fresh 12-way head.
    model = models.__dict__[args.arch](pretrained=False)
