...
# Stage-2 (self-supervised branch) of the geometry -> texture curriculum: init from
# the stage-1 28-way-IDEM-geometry SimCLR backbone (train_simclr_28way_IDEM_scratch.py),
# continue NT-Xent training on the 60-way IDEM colorbg *texture* dataset. Both stages
# share the same SimCLR projector shape, so the stage-1 checkpoint loads directly here
# (unlike a barlowtwins/classifier stage-1 checkpoint, which would need a head swap).
...
args.stage1_checkpoint = 'resnet50_simclr_28way_IDEM_seed777_model_best.pth.tar'

args.lr = 3e-4
args.epochs = 200

args.data_root = '/mnt/smb/locker/issa-locker/users/Seojin/data/face_data/vbsle_50k_texture_colorbg/60way_IDEM_colorbg/'
args.filename_prefix = f'{args.arch}_simclr_60way_IDEM_colorbg_from_simclr28way_seed{args.seed}_'
