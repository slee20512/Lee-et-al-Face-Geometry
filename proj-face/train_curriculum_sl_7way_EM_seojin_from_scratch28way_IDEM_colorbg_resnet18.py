...
# fine-tuning on the plain (no colorbg) 7-way emotion dataset -- same lineage as
# train_curriculum_sl_7way_EM_seojin_colorbg_from_scratch28way_IDEM_colorbg_resnet18.py
# but without the colorbg->colorbg domain match, so this one and that one isolate the
# colorbg-vs-plain target effect while holding the stage-1 backbone fixed.
...
args.filename_prefix = f'{args.arch}_curriculum_sl_7way_EM_seojin_from_scratch28way_IDEM_colorbg_seed{args.seed}_'
...
if os.path.exists(latest_filename):
    ...
    # stage-1 checkpoint is 28-way -- build with that head to match the saved
