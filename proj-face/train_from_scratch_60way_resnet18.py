...
# ResNet-18 ablation of train_from_scratch_60way.py (supervised branch): same random-init
# SGD schedule on the full 60-way IDEM colorbg dataset, but resnet18 instead of resnet50.
# Larger nets like resnet50 tend to be hard to train from scratch on our experimental
# datasets (much smaller than ImageNet); dropping to resnet18 to match dataset size.
# Companion to train_simclr_60way_resnet18.py (the SimCLR side of the same arch swap).
...
# from-scratch schedule: same as train_from_scratch_60way.py
args.lr = 0.1
args.epochs = 100
args.lr_milestones = [30, 60, 90]

args.filename_prefix = f'{args.arch}_scratch_60way_IDEM_colorbg_seed{args.seed}_'
