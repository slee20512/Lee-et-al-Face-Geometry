...
# Curriculum learning, stage 2 (supervised branch), resnet18: init from the colorbg
# 28-way IDEM from-scratch backbone (train_from_scratch_28way_IDEM_colorbg_resnet18.py,
# SL_resnet18_scratch_28way_IDEM_colorbg_seed777_model_best), continue supervised
# fine-tuning on the plain (geometry-only, no colorbg) version of the same 28-way IDEM
# identity task -- the reverse domain direction of
# train_curriculum_sl_28way_IDEM_colorbg_from_scratch28way_IDEM.py (which went
# plain -> colorbg on resnet50). Same class count as stage 1 (28).
...
args.stage1_checkpoint = 'resnet18_scratch_28way_IDEM_colorbg_seed777_model_best.pth.tar'
args.stage1_num_classes = 28
...
args.filename_prefix = f'{args.arch}_curriculum_sl_28way_IDEM_from_scratch28way_IDEM_colorbg_seed{args.seed}_'
...
if os.path.exists(latest_filename):
    ...
    # stage-1 checkpoint is a 28-way classifier -- build with that head to match the
    # saved state_dict, then swap in a fresh 28-way head (new domain, same class count).
    model = models.__dict__[args.arch](pretrained=False)
