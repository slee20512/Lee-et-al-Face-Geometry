...
# ImageNet-pretrained fine-tune on the 28-way IDEM colorbg dataset -- resnet18 companion
# to train_finetune_28way_IDEM_resnet18.py (same recipe, run on the plain vbsle_50k/28way_IDEM
# instead), and the single-dataset counterpart of train_finetune_28way_combined_resnet18.py
# (which merges both). Same fine-tuning recipe as train_finetune_60way.py.
...
args.pretrained = True  # start from ImageNet weights, unlike train_from_scratch_28way_IDEM_colorbg_resnet18.py
...
args.filename_prefix = f'{args.arch}_finetune_28way_IDEM_colorbg_seed{args.seed}_'
