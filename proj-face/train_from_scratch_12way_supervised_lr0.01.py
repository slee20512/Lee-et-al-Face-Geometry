...
# lr ablation of train_from_scratch_12way_supervised.py: same random-init SGD setup
# on the 12-way identity dataset, but lr=0.01 instead of 0.1 -- the lr=0.1 schedule
# (borrowed from train_from_scratch_60way.py, tuned for a much larger dataset) got
# stuck at chance-level accuracy/loss (~8.3% / ln(12)) for 60+ epochs on this smaller
# 12-way set. Separate checkpoint from the lr=0.1 run; doesn't touch it.
...
args.filename_prefix = f'{args.arch}_scratch_vbsle_50k_12way_lr0.01_seed{args.seed}_'
