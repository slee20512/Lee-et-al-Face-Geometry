...
# ResNet-18 ablation of train_from_scratch_12way_supervised.py: same random-init SGD
# setup on the 12-way identity dataset, but resnet18 instead of resnet50. Both the
# resnet50 lr=0.1 and lr=0.01 attempts converged to exactly chance-level
# accuracy/loss (~8.3% / ln(12)) over all 100 epochs -- ruling out learning rate as
# the issue. This tests whether a smaller network (matching the original hypothesis
# that resnet50 is hard to train from scratch on our smaller experimental datasets)
# can actually learn this task from random init where resnet50 couldn't.
...
args.filename_prefix = f'{args.arch}_scratch_vbsle_50k_12way_seed{args.seed}_'
