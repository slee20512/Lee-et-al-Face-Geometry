...
# WSD (warmup-stable-decay) ablation of train_from_scratch_28way_IDEM_resnet18.py: same
# resnet18 from-scratch run on the plain (geometry-only, no colorbg) 28-way IDEM dataset,
# but linear warmup + stable + linear decay instead of plain MultiStepLR. The plain
# MultiStepLR version (lr=0.1, no warmup) collapsed to chance accuracy from step 1 --
# same failure mode already seen on this exact dataset in
# train_curriculum_sl_28way_IDEM_from_scratch_texture_colorbg_12way_ver2_warmup.py, whose
# fix was warmup + decay (there: Adam + cosine; here: plain SGD + linear WSD instead, to
# isolate whether warmup alone -- without also switching optimizer -- is enough).
...
# WSD schedule: linear warmup, stable at peak lr, linear decay to 0 over the last stretch
args.lr = 0.1
args.epochs = 100
args.warmup_epochs = 5
args.decay_epochs = 20  # stable from warmup_epochs..(epochs - decay_epochs), then decays

args.filename_prefix = f'{args.arch}_scratch_28way_IDEM_wsd_seed{args.seed}_'
