...
# Curriculum learning, stage 2 (supervised branch), 12-way-identity-from-scratch
# lineage: same structure as train_curriculum_sl_7way_EM_seojin.py (init backbone,
# fresh 7-class head, SGD), but the stage-1 checkpoint here is
# train_from_scratch_12way_supervised.py (random-init SGD on the 12-way identity
# dataset) instead of the 60-way-IDEM-from-scratch backbone. Same lr/epochs as the
# other 7way_EM_seojin curriculum runs for a like-for-like comparison across lineages.
...
args.stage1_checkpoint = 'resnet50_scratch_vbsle_50k_12way_seed777_model_best.pth.tar'
...
args.filename_prefix = f'{args.arch}_curriculum_sl_7way_EM_seojin_from_scratch12way_lr3e-4_seed{args.seed}_'
...
if os.path.exists(latest_filename):
    ...
    # stage-1 checkpoint is 12-way -- build with that head to match the saved
