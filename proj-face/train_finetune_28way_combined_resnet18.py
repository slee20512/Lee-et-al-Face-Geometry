...
# ImageNet-pretrained fine-tune ablation of train_from_scratch_28way_combined_resnet18.py:
# same *combined* 28-way IDEM dataset -- ConcatDataset of the plain vbsle_50k/28way_IDEM set
# with the vbsle_50k_texture_colorbg/28way_IDEM_colorbg set (same 28 identity x emotion
# classes in both, verified matching folder names -- ImageFolder's class_to_idx is computed
# by sorting folder names, so both instances get identical label indices and can be
# concatenated directly) -- but starting from ImageNet weights (like train_finetune_60way.py)
# instead of random init, with resnet18 to match the from-scratch companion script.
...
args.pretrained = True  # start from ImageNet weights, unlike train_from_scratch_28way_combined_resnet18.py
...
# fine-tuning schedule: much lower lr than from-scratch, converges in far fewer epochs
# since the backbone already has useful features from ImageNet pretraining
args.lr = 0.001
args.epochs = 30
args.lr_milestones = [15, 25]

args.filename_prefix = f'{args.arch}_finetune_28way_IDEM_combined_seed{args.seed}_'
...
# init model -- ImageNet-pretrained backbone, fresh head for our class count
