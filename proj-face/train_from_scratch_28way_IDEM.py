...
# Supervised from-scratch training on the plain (geometry-only, no colorbg) 28-way IDEM
# dataset -- companion to train_from_scratch.py (same recipe, run on
# vbsle_50k_texture_colorbg/28way_IDEM_colorbg instead) and to
# train_simclr_28way_IDEM_scratch.py (the SimCLR side of the same dataset).
...
# from-scratch schedule: higher starting lr + multi-step decay over many more epochs than fine-tuning needs
args.lr = 0.1
args.epochs = 100
