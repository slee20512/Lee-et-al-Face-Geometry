...
# Recipe ablation of train_curriculum_sl_28way_IDEM_from_scratch_texture_colorbg_12way_ver2.py:
# same cross-domain curriculum (textured 12-way ver2 backbone -> geometry-only 28-way
# IDEM), but Adam + linear warmup + cosine decay instead of plain SGD + MultiStepLR --
# same recipe change as the 7-way emotion counterpart, tried here for the same reason
# (plain-SGD version stuck at chance through epoch 1, with an lr drop at epoch 10
# baked in that would make recovery even less likely).
...
args.stage1_checkpoint = 'resnet50_scratch_texture_colorbg_12way_ver2_seed777_model_best.pth.tar'
args.stage1_num_classes = 12

args.data_root = '/mnt/smb/locker/issa-locker/users/Seojin/data/face_data/vbsle_50k/28way_IDEM/'
args.seed = 777

args.lr = 3e-4  # Adam lr -- same numeric value as the SGD curriculum recipe, different optimizer
args.epochs = 20
args.warmup_epochs = 2

args.filename_prefix = f'{args.arch}_curriculum_sl_28way_IDEM_from_scratch_texture_colorbg_12way_ver2_warmup_seed{args.seed}_'
...
def lr_lambda(epoch):
    ...
    return 0.5 * (1 + math.cos(math.pi * progress))  # cosine decay 1x -> 0
...
if os.path.exists(latest_filename):
    ...
    # stage-1 checkpoint is a 12-way classifier -- build with that head to match the
    # saved state_dict, then swap in a fresh 28-way head.
    model = models.resnet50(pretrained=False)
