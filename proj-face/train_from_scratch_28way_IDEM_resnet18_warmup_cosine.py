...
# Third ablation of train_from_scratch_28way_IDEM_resnet18.py's stuck-at-chance failure
# (see that file, and the WSD ablation train_from_scratch_28way_IDEM_resnet18_wsd.py):
# this one replicates the fix that actually worked before on this exact dataset in
# train_curriculum_sl_28way_IDEM_from_scratch_texture_colorbg_12way_ver2_warmup.py --
# Adam + linear warmup + cosine decay instead of plain SGD + MultiStepLR -- rather than
# just changing the decay shape under SGD like the WSD ablation does. Same lr/weight_decay
# as that precedent; warmup_epochs and epochs scaled up since this is a full from-scratch
# run (100 epochs) rather than a short fine-tune off a pretrained backbone (20 epochs).
...
args.batch_size = 128
args.print_freq = 30
args.weight_decay = 1e-6  # matches the Adam precedent, not the SGD 1e-4 used elsewhere
args.num_workers = 8

args.data_root = '/mnt/smb/locker/issa-locker/users/Seojin/data/face_data/vbsle_50k/28way_IDEM/'
args.seed = 777

args.lr = 3e-4  # Adam lr -- matches the warmup precedent, not the SGD 0.1 used elsewhere
args.epochs = 100
args.warmup_epochs = 5

args.filename_prefix = f'{args.arch}_scratch_28way_IDEM_warmup_cosine_seed{args.seed}_'
