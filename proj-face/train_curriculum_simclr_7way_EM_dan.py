...
# Curriculum learning, stage 2 (self-supervised branch): take the SimCLR
# 60-way-IDEM backbone (train_simclr_60way.py's SL_resnet50_simclr_60way_IDEM_colorbg_seed777_model_best)
# and continue SimCLR (contrastive, unlabeled) training at a much lower lr on one
# identity's 7-way emotion images. No label swap needed -- SimCLR never used the
# 60-way head as classes, just as augmented-pair contrast, so the projector carries
# over as-is and keeps training on the new (smaller, single-identity) image set.
...
args.stage1_checkpoint = 'resnet50_simclr_60way_IDEM_colorbg_seed777_model_best.pth.tar'

# much lower lr than the stage-1 schedule (3e-4) and far fewer epochs -- the
# backbone/projector are already specialized for this face-ID/emotion domain, we're
# just lightly adapting to one identity's 7 emotions.
args.lr = 3e-5
args.epochs = 50

args.data_root = '/mnt/smb/locker/issa-locker/users/Seojin/data/face_data/vbsle_50k_texture_colorbg/7way_EM_dan_colorbg/'
args.filename_prefix = f'{args.arch}_curriculum_simclr_7way_EM_dan_colorbg_from_simclr60way_seed{args.seed}_'
