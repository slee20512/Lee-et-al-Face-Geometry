...
# ResNet-18 ablation of train_from_scratch_28way_IDEM.py (supervised branch): same
# random-init SGD schedule on the plain (geometry-only, no colorbg) 28-way IDEM dataset,
# but resnet18 instead of resnet50. Larger nets like resnet50 tend to be hard to train
# from scratch on our experimental datasets (much smaller than ImageNet); dropping to
# resnet18 to match dataset size, mirroring train_from_scratch_60way_resnet18.py.
...
# from-scratch schedule: same as train_from_scratch_28way_IDEM.py
args.lr = 0.1
args.epochs = 100
args.lr_milestones = [30, 60, 90]

args.filename_prefix = f'{args.arch}_scratch_28way_IDEM_seed{args.seed}_'
