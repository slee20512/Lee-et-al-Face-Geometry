...
# ImageNet-pretrained finetune version of the 12-way identity stage-1 task, matching
# train_finetune_60way.py's recipe. Both train_from_scratch_12way_supervised.py
# (lr=0.1) and its lr=0.01 ablation trained a random-init ResNet-50 to exactly
# chance-level accuracy/loss (~8.3% / ln(12)) over all 100 epochs -- ruling out
# learning rate as the issue. Starting from ImageNet-pretrained weights instead of
# random init, as this dataset is too small to learn good features from scratch.
...
args.pretrained = True  # start from ImageNet weights, unlike train_from_scratch_12way_supervised.py
...
args.filename_prefix = f'{args.arch}_finetune_vbsle_50k_12way_seed{args.seed}_'
