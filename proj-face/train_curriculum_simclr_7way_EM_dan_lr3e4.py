...
# Curriculum learning, stage 2 (self-supervised branch), lr ablation: same setup as
# train_curriculum_simclr_7way_EM_dan.py (init from the SimCLR 60-way-IDEM backbone,
# continue NT-Xent training on one identity's 7-way emotion images), but at the
# stage-1 schedule's original lr (3e-4) instead of 3e-5. Diagnostic run: the 3e-5
# version's loss plateaued around 2.90 from epoch 0 through epoch 12 without moving --
# this checks whether that lr was simply too low to make progress in the epoch
# budget, or whether the plateau is a real floor (harder in-batch negatives from a
# single identity) that persists regardless of lr.
...
args.filename_prefix = f'{args.arch}_curriculum_simclr_7way_EM_dan_colorbg_from_simclr60way_lr3e-4_seed{args.seed}_'
